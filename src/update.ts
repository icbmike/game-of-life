...
function playModeUpdate(game: Game, delta: number, ctx: CanvasRenderingContext2D) {
    const {cells, generationLength} = game;
    generationAge += delta;
    ...
    if (generationAge > generationLength) {
        ...
        for (let i = 0; i < cells.getSize(); i++) {
            for (let j = 0; j < cells.getSize(); j++) {
                const neighbours = cells.getNeighbours(i, j);
        ...
        cells.setLiveCells(nextGenerationCells);
    }
}
